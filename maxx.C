...
#include <TMultiGraph.h> //Несколько гарфиков на одном
...
int maxx()
{
	...
	TString PathToRawEvents = Form("/home/user/gvd/DATA/%04d/k%04d.raw.events.%d.%04d.root", fileNum, RunNumber, Sdc, fileNum);
		TFile FileRawEvents(PathToRawEvents);
		
	TString fOutWaveFileName = Form("/home/user/max/maxx_%d_%04d.root", Sdc, fileNum);
...
// Построение графика - зависимость амплитуды импульса от времени на текущем канале					
				...
				//	WaveFormGraph->SetName(Form("Tutya"));
				//	WaveFormGraph->SetTitle(Form("Tutya"));
				...
				// Цикл по отсчетам АЦП от 0 до длины импульса в кодах АЦП
					for(Int_t n = 0; n < nbins; n++) {
						WaveFormGraph->SetPoint(n, (n*0.000005+EventTime+offset*0.000005), data[n]-pedestal);	
					}
				}
				
			// Подписи осей		
				//	WaveFormGraph->GetXaxis()->SetTitle("EventTime, FADC counts");
				//	WaveFormGraph->GetYaxis()->SetTitle("amplitude, FADC counts");
				//	fWaveout.cd();
			// Запись графика в файл		
				//	WaveFormGraph->Write();
				
					mg->Add(WaveFormGraph);
